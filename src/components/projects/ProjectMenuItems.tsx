...
const ProjectMenuItems = ({projectId, userIsManager}:ProjectMenuItemsProps) => {
  ...
  return (
    <Menu as="div" className="relative">
          ...
          >
            <MenuItems className="text-xs absolute right-0 top-8 z-10 min-w-44 rounded-xl overflow-hidden border border-[#2d3348] bg-[#252d3d] shadow-[0_8px_24px_rgba(0,0,0,0.4)] focus:outline-none">
              <MenuItem>
                <Link
                  to={`/projects/${projectId}`}
                  className="flex items-center gap-3 w-full px-4 py-2.5 text-slate-300 transition-colors duration-100 data-focus:bg-[#2d3a4f]"
                >
