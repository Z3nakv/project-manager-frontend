...
const ProjectCard = ({ project, user }: ProjectCardProps) => {
 ...
 const deadline = useMemo(() => {
  ...
  <div className="relative pt-4">
    ...
    <div className="absolute top-0 left-4 w-16 h-4 bg-[#0f1117] border border-zinc-800 border-b-0 rounded-t-md" />
    ...
    <li className="font-mono relative bg-[#0f1117] border border-zinc-800 rounded-tl-sm rounded-tr-2xl rounded-b-2xl p-3.5 hover:border-indigo-500/40 cursor-pointer group shadow-md hover:-translate-y-1 transition-transform duration-150 list-none">
      ...
      <div className="flex items-center justify-between mb-2.5">
        ...
        <ProjectMenuItems projectId={project._id} userIsManager={userIsManager} />
      </div>
          ...
          {Object.entries(tasksByStatus).map(([status, tasks]) => {
            ...
            return (
              <li
                ...
                title={status}
              >
                <span style={{ color: config.color }} className="text-sm leading-none">
                  <config.icon/>
                </span>
      ...
      {upcomingDeadline && (
        <p className="text-[11px] text-slate-500 mt-2.5">
          <span className={`${upcomingDeadline.color} ${upcomingDeadline.bg} px-1.5 py-0.5 rounded-md`}>
            Próxima entrega
          </span>{" "}
          {formatDate(deadline)}
        </p>
