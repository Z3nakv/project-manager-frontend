...
describe("useAuthMutation", () => {
  ...
  describe("useCreateAccountMutation", () => {
    it("debe mostrar toast y resetear el form al crear la cuenta", async () => {
      vi.mocked(createAccount).mockResolvedValue(
        "Cuenta creada correctamente" as any,
      );
      const mockReset = vi.fn();
      const { wrapper } = createWrapper();

      const { result } = renderHook(
        () => useCreateAccountMutation({ reset: mockReset }),
        { wrapper },
      );
    ...
    it("debe mostrar toast de error y NO resetear el form si falla", async () => {
      vi.mocked(createAccount).mockRejectedValue(
        new Error("El usuario ya está registrado"),
      );
      const mockReset = vi.fn();
      const { wrapper } = createWrapper();
  ...
  describe("useAuthenticateUserMutation", () => {
    it("debe limpiar la query de user y navegar al dashboard tras login exitoso", async () => {
      ...
      const { result } = renderHook(() => useAuthenticateUserMutation(), {
        wrapper,
      });
    ...
    it("debe mostrar toast de error si las credenciales son incorrectas", async () => {
      ...
      await waitFor(() =>
        expect(toast.error).toHaveBeenCalledWith("Password incorrecto"),
      );
  ...
  describe("useValidateTokenMutation", () => {
    it("debe llamar a setIsValidToken(true) si el token es válido", async () => {
      ...
      const { result } = renderHook(
        () => useValidateTokenMutation({ setIsValidToken }),
        { wrapper },
      );
      ...
      await waitFor(() =>
        expect(toast.success).toHaveBeenCalledWith("Token válido"),
      );
    ...
    it("NO debe llamar a setIsValidToken si el token es inválido", async () => {
      ...
      await waitFor(() =>
        expect(toast.error).toHaveBeenCalledWith("Token no válido"),
      );
  ...
  describe("useUpdatePasswordWithTokenMutation", () => {
    it("debe navegar a login tras actualizar el password", async () => {
      vi.mocked(updatePasswordWithToken).mockResolvedValue(
        "Password actualizado" as any,
      );
      const { wrapper } = createWrapper();
      ...
      await waitFor(() =>
        expect(toast.success).toHaveBeenCalledWith("Password actualizado"),
      );
  ...
  describe("useForgotPasswordMutation", () => {
    it("debe resetear el form al enviar el email de recuperación", async () => {
      ...
      const { result } = renderHook(
        () => useForgotPasswordMutation({ reset: mockReset }),
        { wrapper },
      );
      ...
      await waitFor(() =>
        expect(toast.success).toHaveBeenCalledWith("Revisa tu email"),
      );
      expect(mockReset).toHaveBeenCalledTimes(1);
    });
  });
  ...
  describe("useGoogleAuthMutation", () => {
    it("debe guardar el token, cachear el usuario y navegar al dashboard", async () => {
      ...
      expect(getAccessToken()).toBe("google-jwt-token");
      expect(setQueryDataSpy).toHaveBeenCalledWith(["user"], {});
      expect(mockNavigate).toHaveBeenCalledWith("/dashboard", {
        replace: true,
      });
    });
    ...
    it("debe mostrar toast de error si falla la autenticación con Google", async () => {
      vi.mocked(googleAuth).mockRejectedValue(
        new Error("Token de Google inválido"),
      );
      const { wrapper } = createWrapper();
      ...
      await waitFor(() =>
        expect(toast.error).toHaveBeenCalledWith("Token de Google inválido"),
      );
