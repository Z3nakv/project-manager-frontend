  ...
  return (
    <div className="relative pt-4 w-full max-w-sm m-auto">
      ...
      <div className="absolute z-100 top-0 left-5 h-7 flex items-center gap-1.5 bg-bg border border-border-subtle border-b-0 rounded-t-md px-3.5">
        <FiKey className="h-3 w-3 text-accent" />
        <span className="font-mono text-xs text-text-muted">recuperar.auth</span>
      </div>
      ...
      <div className="relative bg-surface-base border border-border rounded-tl-sm rounded-tr-2xl rounded-b-2xl shadow-lifted p-8 overflow-hidden">
        ...
        {/* Header */}
        <div className="mb-6">
          <h1 className="text-xl font-bold text-text-primary">Restablecer password</h1>
        ...
        <div className="flex flex-col gap-2 mt-6">
          ...
          <p className="text-center text-sm text-text-muted">
            ¿No tienes cuenta?{" "}
            <Link to="/auth/register" className="text-accent hover:text-ring font-medium transition-colors duration-150">
              Crea una
            </Link>
          </p>
        </div>
